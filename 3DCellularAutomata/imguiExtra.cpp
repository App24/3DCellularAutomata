...
bool ImGui::ColorPicker(const char* label, float col[3])
{
    ...
    if (ImGui::IsItemActive() && ImGui::GetIO().MouseDown[0])
    {
        ...
        if (!(mouse_pos_in_canvas.x < -10 || mouse_pos_in_canvas.x >= SV_PICKER_SIZE.x + 10 || mouse_pos_in_canvas.y < -10 || mouse_pos_in_canvas.y >= SV_PICKER_SIZE.y + 10)) {
    ...
    if ((ImGui::IsItemHovered() || ImGui::IsItemActive()) && ImGui::GetIO().MouseDown[0])
    {
        ...
        if (!(mouse_pos_in_canvas.y < -10 || mouse_pos_in_canvas.y >= SV_PICKER_SIZE.y + 10)) {
